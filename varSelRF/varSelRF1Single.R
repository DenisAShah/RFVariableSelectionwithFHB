...
# * This script uses a single train/test split
# 
# 
# --------------------------------------------------------
...
# ## No. unique variables
## ----vars-num-unique---------------------------------------------------------------
# How many unique variables were used?
vars %>%
  dplyr::distinct() %>%
  dplyr::arrange(var) %>%
...
# Load the object (m1) containing the information on the models (from script `varSelRF0.Rmd`):
load("varSelRFResII.RData")
...
save(bayes_probs, file = "varSelRF1TestRes.RData")

# 
# 
## ----load-test-data-fitted-probs---------------------------------------------------
# Load the bayes_probs object:
...
CalculateStats <- function(.x) {
  ...
  # Args:
  #  .x = a nested object containing predicted probs for each model
  # Returns:
