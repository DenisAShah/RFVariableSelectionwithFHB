# # Objective(s)
# * 38 RF models from the VSURF algorithm
...
# 
## ----Setup-----------------------------------------------------------------------------
# Load the object (vsurf) containing the information on the models (see VSURF0.Rmd):
...
# vars_sub is a character vector of the 27 distinct weather variables (resist is excluded) used among the 38 RF models
vars_sub <-
  vsurf.filtered %>%
  dplyr::select(vars) %>%
...
# use `hardhat::extract_parameter_dials()` instead
pluck(fhb_set, "option", 2, "param_info") %>% hardhat::extract_parameter_dials("mtry")
...
tic()
VSURFRep <- future_map(my.seeds, model_metrics, .options = furrr_options(seed = TRUE))  
# 68590.17 sec = 1143 min = 19.05 hr
toc()

plan(sequential)

save(VSURFRep, file = "VSURFRep.RData")
...
# Example of use: 
get_estimates(x = VSURFRep, .m = auc)
